...
@Component({
  selector: 'ev-info-chart',
  templateUrl: './ev-info-chart.component.html',
  styleUrls: ['./ev-info-chart.component.css']
})
export class EvInfoChartComponent implements OnInit {
  ...
  changeYearChart2(yearValue: number) {
    this.evDashboardService.getEvsBarChart(yearValue).then((res) => {
      ...
      this.chart2.data.datasets[2].data = this.getCurrentMonthData(yearValue);
      this.chart2.update();
    });
  }
